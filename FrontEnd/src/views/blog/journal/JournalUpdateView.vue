  ...
  <div v-else>
    <div class="container">
      <div class="header">
        ...
        <h3>수정하기</h3>
        <a @click="saveJournal" class="btn lined-bg f-color-c h-lined-c a-solid-c">
      ...
      <div class="content">
        <div class="title" v-if="journal">
          <div>
            ...
            <b>{{ journal.createdDate }}</b>
          </div>
          <div>{{ journal.content.length }} 자</div>
        </div>
        <textarea class="input focus-lined-c" placeholder="미리보기" v-model="journal.content" required></textarea>
      </div>
    </div>
  </div>
</template>
