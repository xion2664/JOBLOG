...
export const useEssayResumeStore = defineStore("essayResume", {
  ...
  actions: {
    ...
    async deleteEssay(id) {
      ...
      try {
        ...
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/essay/${authStore.userInfo.sub}`,
          config
        );
    ...
    async spellCheck(essay) {
      const authStore = useAuthStore();
      await authStore.updateUserInfoFromToken();
      const newEssay = essay.replace(/\n/g, " ");
      const sentence = {
        text: newEssay,
      };
      const config = {
      ...
      try {
        const res = await axios.post(`${import.meta.env.VITE_API_BASE_URL}/check-spelling`, sentence, config);
        this.spellChecked = res.data;
      } catch (err) {
        console.error(err);
      }
    },
    ...
    async createCategory(category) {
      ...
      try {
        ...
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/category/${authStore.userInfo.sub}`,
          config
        );
